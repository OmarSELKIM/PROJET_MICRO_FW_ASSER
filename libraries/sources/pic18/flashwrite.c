...
#define _TBLPTR_as_ptr		(*((far unsigned char **)&TBLPTR))
...
volatile const unsigned char _flash_buffer_j[_FLASH_ERASE_SIZE] @ 0x400;
...
void flash_write(const unsigned char * source, unsigned int length, far unsigned char * dest){
	...
	while(length){
		...
		_TBLPTR_as_ptr = (unsigned char *)_flash_buffer_j;	// first erase the holding buffer
		EECON1bits.FREE=1;
		initiate_write();
		...
		for(ix=0; ix != _FLASH_ERASE_SIZE; ){
			if(offset || (length==0)){	// not reading from source, read backup from dest
				if(offset)
					offset--;
				_TBLPTR_as_ptr=(unsigned char *)ptr;
				asm("tblrd*");
			}else{
			...
			_TBLPTR_as_ptr = (unsigned char *)&_flash_buffer_j[ix];
			transfer(++ix);	// commit what we have to temp buffer
		}

		_TBLPTR_as_ptr = (unsigned char *)dest;	// first erase the destination sector
		EECON1bits.FREE=1;
		initiate_write();
		...
		for(ix=0; ix != _FLASH_ERASE_SIZE; ){
			...
			_TBLPTR_as_ptr = (unsigned char *)dest++;	// point to destination
...
void flash_write(const unsigned char * source_addr,unsigned int length,far unsigned char * dest_addr)
{
	...
	_TBLPTR_as_ptr = dest_addr;	// initially point to destination sector
	...
	while(length){
		...
		do{
			...
			if((index>=offset)&&(length)){	// or specifed data area
				...
				_TBLPTR_as_ptr = dest_addr + index;	// point back to destination sector
				length--;
			}
#else
			if((index>=offset)&&(length)){	// or specifed data area
